...
#         ]
#     )
#     llm = langchain_chat_models.ChatOpenAI(
#         model="gpt-4o-mini", 
#         temperature=0.1,
#         openai_api_key=""
#         )  # type: ignore
...
def invoke(
    ...
    if chat_history:
        return _invoke_with_chat_history(chat_history=chat_history, query=query)
    return _invoke(query=query)
...
def _invoke(query: str) -> str:
    return f"¡Hola!, Respuesta simulada a: {query}"
...
def _invoke_with_chat_history(
    ...
    historial = "\n".join(
        f"{m.sender}: {m.message}" for m in chat_history.messages
    )
    return f"¡Hola!, Simulado con historial:\n{historial}\n\nRespuesta a: {query}"
